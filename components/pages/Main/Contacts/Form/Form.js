...
function Form({ data, siteTitle }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await axios.post(`https://api.lookout.consulting/forms/contacts/`, bodyFormData)
    ...
    return (
        <div className={classes.Form}>
            {data?.formTitle && <div className={classes.Title} dangerouslySetInnerHTML={{ __html: data.formTitle }} />}
            ...
            <div className={classes.Control}>
                ...
                {data?.btnSend && <Button className={classes.BtnSend}>{data.btnSend}</Button>}
            </div>
        </div>
    )
}
